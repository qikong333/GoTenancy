...
func (u *Users) SignUp(email, password string) (*model.Account, error) {
	...
	err := u.DB.QueryRow(`
		INSERT INTO GoTenancy_accounts(
			email, 
			stripe_id, 
			subscription_id, 
			plan, 
	...
	_, err = u.DB.Exec(`
		INSERT INTO GoTenancy_users(account_id, email, password, token, role)
		VALUES($1, $2, $3, $4, $5)
...
func (u *Users) Auth(accountID int64, token string, pat bool) (*model.Account, *model.User, error) {
	...
	user := &model.User{}
	row := u.DB.QueryRow("SELECT * FROM GoTenancy_users WHERE account_id = $1 AND token = $2", accountID, token)
...
func (u *Users) GetDetail(id int64) (*model.Account, error) {
	...
	row := u.DB.QueryRow("SELECT * FROM GoTenancy_accounts WHERE id = $1", id)
	...
	rows, err := u.DB.Query("SELECT * FROM GoTenancy_users WHERE account_id = $1", id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (u *Users) GetUserByEmail(email string) (*model.User, error) {
	user := &model.User{}
	row := u.DB.QueryRow("SELECT * FROM GoTenancy_users WHERE email = $1", email)
...
func (u *Users) GetByStripe(stripeID string) (*model.Account, error) {
	var accountID int64
	row := u.DB.QueryRow("SELECT id FROM GoTenancy_accounts WHERE stripe_id = $1", stripeID)
