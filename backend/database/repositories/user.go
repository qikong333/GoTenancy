...
func NewUserRepository(source map[uint]models.User) UserRepository {
...
func (r *userMemoryRepository) Select(query Query) (user models.User, found bool) {
	...
	// set an empty User if not found at all.
	if !found {
		user = models.User{}
	}
...
// SelectMany same as Select but returns one or more User as a slice.
